...
test('grid click handler builds selected module and updates UI state', () => {
  ...
  const firstCell = document.getElementById('grid').children[0];
  firstCell.className = 'cell empty';
  document.getElementById('grid').dispatchEvent({ type: 'click', target: firstCell });
  ...
  assert.ok(game.grid.cells[0].buildingTypeId === generatorType.id);
